...
struct transform {
  ...
  Eigen::Translation3d translation; // 3 个 double 类型的浮点数 用于记录xyz坐标
  Eigen::Quaterniond rotation; // 四元数 [w, x, y, z] 储存
};
 ...
 * as it goes out of scope. The class is not thread-safe. 带有作用域计数器的类， 
 *
 * Example
 * @code
...
struct transform_buffer {
  ...
  /// @brief 将变换插入缓冲区。
  ///
  /// @param _parent_frame 父连杆的坐标系 ID。
  /// @param _child_frame 子连杆的坐标系 ID。
  /// @param _stamp_time 变换的时间戳(仅适用于动态连杆)。
  /// @param _tf 变换数据(4x4 坐标系变换矩阵)。
  /// @param _is_static 标志，指示连杆是动态还是静态。
  ///
  /// @throw std::runtime_error 如果会导致变换树结构非法。
  /// @throw std::bad_variant_access 如果连杆的类型(动态/静态)发生改变。
  void
  ...
  /// @brief 将变换插入缓冲区。 
  ///
  /// @param _tf 变换数据(transform结构体 分开储存旋转矩阵和平移向量)
  void
  set(const std::string& _parent_frame, const std::string& _child_frame,
  ...
  /// @brief 获取变换。 
  ///
  /// @param _target_frame 目标连杆的坐标系 ID。 
  /// @param _source_frame 源连杆的坐标系 ID。 
  /// @param _query_time 查询时间（仅适用于动态连杆）。 
  /// @param _tolerance 查询等待的超时时间。 
  ///
  /// @throw std::runtime_error 如果无法获取变换，则抛出此异常。
  Eigen::Isometry3d
