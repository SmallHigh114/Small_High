#include "transform_coordinate/fast_tf.hpp"
  ...
  /// @brief Creates a joined transform from the splitted data.  把平移向量和旋转向量合并为 Eigen::Isometry3d 类型的 4*4 矩阵
  /// @return Eigen::Isometry3d = translation * rotation 
  static Eigen::Isometry3d
  join(const transform& _split) noexcept {
    return Eigen::Isometry3d{_split.translation * _split.rotation};
  }

  /// @brief Creates a split transform from the joined data. Eigen::Isometry3d 类型的 4*4 矩阵拆分为平移向量和旋转向量
  static transform
  ...
  /// @brief 动态变换 为数据添加时效性，如果有效时间就需要重新计算或者插值
  ...
  /// @brief 把带有时间戳的新位姿存进去，同时把超过有效期（过期）的老数据自动踢出去。
  void
  ...
  /// @brief returns the vector interpolation between _l and _r. 返回 _l 和 _r 之间的向量插值。
  /// 三维向量插值
  /// @param _l left value.左值
  /// @param _r right value.右值
  ...
  /// @brief 查找_time时间的位姿，如果没有则使用二分法估测中间值
  /// @param _time 
  /// @return 
  Eigen::Isometry3d
  ...
  /// @brief 结合父节点，如果成功就不必结合子节点，如果失败则尝试结合子节点
  /// @param _parent_frame 
  /// @param _child_frame 
  void
  ...
  /// 向一个树状拓扑结构中添加或更新坐标系之间的变换关系 支持动态变换或者静态变换
  void
  ...
  /// @brief 计算 source 坐标系 到 target 坐标系 的坐标变换矩阵
  /// @param _target_frame 
  /// @param _source_frame 
  /// @param _query_time 
  /// @param _tolerance 
  ...
  transform_buffer::get(const std::string& _target_frame,
    ...
    // 线程锁
    std::unique_lock<std::mutex> l{m_};
    // find the correct iterators 找到正确的迭代器
...
Eigen::Isometry3d Coordinate::getIsometry3d() {
...
Eigen::Quaterniond getQuaterniond(const RpyAngle & rpy_angle) {
...
void CoordinateInit(transform_buffer & buf, Coordinate & cod, const bool & _is_static) {
  buf.set(
    cod.parent_frame_id,
    cod.child_frame_id,
    cod.stamp,
